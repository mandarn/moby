...
func (ldm *LayerDownloadManager) Download(ctx context.Context, initialRootFS image.RootFS, layers []DownloadDescriptor, progressOutput progress.Output) (image.RootFS, func(), error) {
	...
	for _, descriptor := range layers {
		...
		logrus.Debugf("Download descriptor diffID: %s", lDiffID)
		key := descriptor.Key()
		transferKey += key
		...
		if !missingLayer {
			...
			if err == nil {
				...
				lChainID := getRootFS.ChainID()
				logrus.Debugf("aoComment: Exists: %s ", diffID)
				ok := ldm.layerStore.Exists(diffID)
				if ok {
					logrus.Debugf("Layer: %s exists on GraphDriver", diffID)
				}
